...
extern "C" __global__ void outproj_residual_kernel(const half_t* __restrict__ input_tensor, half_t* __restrict__ output, const half_t* __restrict__ residual, const half_t* __restrict__ weights);
extern "C" __global__ void __launch_bounds__(128, 3) outproj_residual_kernel(const half_t* __restrict__ input_tensor, half_t* __restrict__ output, const half_t* __restrict__ residual, const half_t* __restrict__ weights) {
  ...
  for (int i_5 = 0; i_5 < 4; ++i_5) {
    tl::cp_async_gs_conditional<16>((&(((half_t*)input_shared)[(((((i_5 * 1024) + ((((int)threadIdx.x) >> 2) * 32)) + (((((((int)threadIdx.x) & 31) >> 4) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 15) >> 3) + (((int)threadIdx.x) & 1)) & 1) * 8)) + 8192)])), (&(input_tensor[(((((((int)blockIdx.y) * 49152) + (i_5 * 12288)) + ((((int)threadIdx.x) >> 2) * 384)) + ((((int)threadIdx.x) & 3) * 8)) + 64)])), (((((((int)blockIdx.y) * 128) + (i_5 * 32)) + (((int)threadIdx.x) >> 2)) < 4693) && ((((((int)blockIdx.y) * 128) + (i_5 * 32)) + (((int)threadIdx.x) >> 2)) < 4693)));
  }
  #pragma unroll
  ...
  for (int ko = 0; ko < 9; ++ko) {
    ...
    for (int i_7 = 0; i_7 < 4; ++i_7) {
      tl::cp_async_gs_conditional<16>((&(((half_t*)input_shared)[(((((((ko + 3) & 3) * 4096) + (i_7 * 1024)) + ((((int)threadIdx.x) >> 2) * 32)) + (((((((int)threadIdx.x) & 31) >> 4) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 15) >> 3) + (((int)threadIdx.x) & 1)) & 1) * 8))])), (&(input_tensor[((((((((int)blockIdx.y) * 49152) + (i_7 * 12288)) + ((((int)threadIdx.x) >> 2) * 384)) + (ko * 32)) + ((((int)threadIdx.x) & 3) * 8)) + 96)])), (((((((int)blockIdx.y) * 128) + (i_7 * 32)) + (((int)threadIdx.x) >> 2)) < 4693) && ((((((int)blockIdx.y) * 128) + (i_7 * 32)) + (((int)threadIdx.x) >> 2)) < 4693)));
    }
    #pragma unroll
...
cudaError_t launchOutProjectionResidualB13(
  ...
  static const cudaError_t attributeStatus = cudaFuncSetAttribute(
    outproj_residual_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, 65536);
  if(attributeStatus != cudaSuccess)
    return attributeStatus;
  dim3 grid(3, 37, 1);
  outproj_residual_kernel<<<grid, 128, 65536, stream>>>(
