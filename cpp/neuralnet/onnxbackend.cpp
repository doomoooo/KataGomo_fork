...
#ifdef _WIN32
// If you have the header, include it. Otherwise we might need to rely on the C API or specific linking.
// Usually standard onnxruntime package on Windows has this.
...
struct ComputeHandle {
      ...
      memoryInfo(Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault))
  {
    ...
    sessionOptions.SetIntraOpNumThreads(32);
    ...
    OrtStatus* status = ortApi.SessionOptionsAppendExecutionProvider_DML(sessionOptions, 0); // device_id = 0
    if (status) {
