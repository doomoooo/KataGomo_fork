...
struct InputBuffers;
// TRT backend contract:
// - InputBuffers are owned by NNServerBuf (one instance per NN server thread).
// - createInputBuffers and createComputeHandle are called on that same thread.
// - We cache the thread's InputBuffers pointer only to bind stable host addresses during cudaGraph pre-capture.
static thread_local InputBuffers* threadLocalInputBuffersForCudaGraph = nullptr;

struct CudaHostArrayDeleter {
  void operator()(float* ptr) const {
    if(ptr != nullptr) {
      cudaError_t status = cudaFreeHost(ptr);
      if(status != cudaSuccess) {
        Global::fatalError(
          string("TensorRT backend: cudaFreeHost failed: ") + cudaGetErrorString(status)
        );
      }
    }
  }
...
using PinnedHostFloatArray = unique_ptr<float[], CudaHostArrayDeleter>;

static PinnedHostFloatArray makePinnedHostFloatArray(size_t eltCount) {
  if(eltCount == 0) {
    return PinnedHostFloatArray(nullptr);
  }
  float* ptr = nullptr;
  CUDA_ERR("makePinnedHostFloatArray", cudaMallocHost(reinterpret_cast<void**>(&ptr), eltCount * sizeof(float)));
  return PinnedHostFloatArray(ptr);
}
...
struct ComputeHandle {
  ...
  struct RuntimeExecState {
    ...
    int lastInputShapeBatch;

    RuntimeExecState()
      : exec(nullptr),
        auxStreams(),
        lastInputShapeBatch(0) {}
  };
  ...
  struct BatchGraphState {
    ...
    BatchGraphState(const BatchGraphState&) = delete;
    BatchGraphState& operator=(const BatchGraphState&) = delete;
  };
  ...
  ComputeHandle(
    ...
    int maxBatchSz,
    bool requireExactNNLen,
    int serverThreadIndex) {
    (void)serverThreadIndex;
    ctx = context;
    this->logger = logger;
    ...
    cudaGraphsPreCaptured = false;
    cudaGraphInputBuffers = nullptr;
    if(this->logger != nullptr && useCudaGraph) {
      this->logger->write("TensorRT backend: cudaGraph is enabled for this execution context");
    }
    ...
    constexpr int builderMaxThreads = 8;
    if(!builder->setMaxThreads(builderMaxThreads)) {
      throw StringError(
        "TensorRT backend: failed to set builder max threads to " + Global::intToString(builderMaxThreads)
      );
    }
    ...
    if (ctx->isOnnx) {
      ...
      profile->setDimensions(
        "input_spatial", OptProfileSelector::kMIN, Dims4(1, spatialC, ctx->nnYLen, ctx->nnXLen)
      );
      profile->setDimensions(
        "input_spatial", OptProfileSelector::kOPT, Dims4(maxBatchSize, spatialC, ctx->nnYLen, ctx->nnXLen)
      );
      profile->setDimensions(
        "input_spatial", OptProfileSelector::kMAX, Dims4(maxBatchSize, spatialC, ctx->nnYLen, ctx->nnXLen)
      );
      ...
      auto applyClassicProfileDimensions = [this, loadedModel](IOptimizationProfile* profile) {
        ...
        profile->setDimensions("InputMask", OptProfileSelector::kMIN, Dims4(1, 1, ctx->nnYLen, ctx->nnXLen));
        profile->setDimensions("InputMask", OptProfileSelector::kOPT, Dims4(this->maxBatchSize, 1, ctx->nnYLen, ctx->nnXLen));
        profile->setDimensions("InputMask", OptProfileSelector::kMAX, Dims4(this->maxBatchSize, 1, ctx->nnYLen, ctx->nnXLen));
        profile->setDimensions(
          "InputSpatial", OptProfileSelector::kMIN, Dims4(1, numInputChannels, ctx->nnYLen, ctx->nnXLen)
        );
        profile->setDimensions(
          "InputSpatial", OptProfileSelector::kOPT, Dims4(this->maxBatchSize, numInputChannels, ctx->nnYLen, ctx->nnXLen)
        );
        profile->setDimensions(
          "InputSpatial", OptProfileSelector::kMAX, Dims4(this->maxBatchSize, numInputChannels, ctx->nnYLen, ctx->nnXLen)
        );
        profile->setDimensions(
          "InputGlobal", OptProfileSelector::kMIN, Dims4(1, numInputGlobalChannels, 1, 1)
        );
        profile->setDimensions(
          "InputGlobal", OptProfileSelector::kOPT, Dims4(this->maxBatchSize, numInputGlobalChannels, 1, 1)
        );
        profile->setDimensions(
          "InputGlobal", OptProfileSelector::kMAX, Dims4(this->maxBatchSize, numInputGlobalChannels, 1, 1)
        );
        if(numInputMetaChannels > 0) {
          profile->setDimensions("InputMeta", OptProfileSelector::kMIN, Dims4(1, numInputMetaChannels, 1, 1));
          profile->setDimensions("InputMeta", OptProfileSelector::kOPT, Dims4(this->maxBatchSize, numInputMetaChannels, 1, 1));
          profile->setDimensions("InputMeta", OptProfileSelector::kMAX, Dims4(this->maxBatchSize, numInputMetaChannels, 1, 1));
        }
      };
      ...
      applyClassicProfileDimensions(primaryProfile);
    ...
    string plan;
    {
      ...
      auto buildPlanCacheFileAndParam = [&](int cacheBatchSize, string& outPlanCacheFile, string& outParamStr) {
        if(ctx->isOnnx) {
          outPlanCacheFile = Global::strprintf(
      ...
      auto tryLoadValidatedPlan = [&](
        const string& candidatePlanCacheFile,
        const string& expectedParamStr,
        bool logInvalidReason,
        string& outPlan
      ) {
        if(!FileUtils::exists(candidatePlanCacheFile)) {
          return false;
        }

        string cachedPlan;
        try {
          cachedPlan = FileUtils::readFileBinary(candidatePlanCacheFile);
        } catch(const StringError& e) {
          (void)e;
          return false;
        };

        if(cachedPlan.size() < 64 + expectedParamStr.size()) {
          if(logInvalidReason && logger != nullptr) {
            logger->write("Could not parse plan, unexpected size in " + candidatePlanCacheFile);
          }
          return false;
        }

        string cachedParamStr = cachedPlan.substr(cachedPlan.size() - expectedParamStr.size());
        string modelHash = cachedPlan.substr(cachedPlan.size() - 64 - expectedParamStr.size(), 64);
        if(modelHash != modelHashStr) {
          if(logInvalidReason && logger != nullptr) {
            logger->write("Plan cache is corrupted or is for the wrong model in " + candidatePlanCacheFile);
          }
          return false;
        }
        if(cachedParamStr != expectedParamStr) {
          if(logInvalidReason && logger != nullptr) {
            logger->write("Plan cache is corrupted or is for the wrong parameters in " + candidatePlanCacheFile);
          }
          return false;
        }

        cachedPlan.erase(cachedPlan.size() - 64 - expectedParamStr.size());
        outPlan = std::move(cachedPlan);
        return true;
      };
    ...
    for(int i = 0; i < engine->getNbIOTensors(); i++) {
      ...
      if(string(name) == "InputMeta") {
        hasInputMetaTensor = true;
      }
    }
    ...
    if(useCudaGraph) {
      ...
      if(threadLocalInputBuffersForCudaGraph == nullptr) {
        throw StringError("TensorRT backend: missing per-thread InputBuffers for cudaGraph pre-capture");
      }
      ensureCudaGraphsPreCaptured(threadLocalInputBuffersForCudaGraph);
    }
  ...
  ~ComputeHandle() {
    ...
    runtimeExecState.exec.reset();
    destroyAuxStreams(runtimeExecState.auxStreams);
    runtimeExecState.lastInputShapeBatch = 0;
  ...
  void validateBatchSize(int batchSize) const {
    if(batchSize <= 0 || batchSize > maxBatchSize) {
      throw StringError("TensorRT backend: invalid batch size");
    }
  }

  void initializeExecutionContext(IExecutionContext* targetExec, vector<cudaStream_t>* targetAuxStreams) {
    bool profileSet = targetExec->setOptimizationProfileAsync(0, cudaStreamPerThread);
    if(!profileSet) {
      throw StringError("TensorRT backend: setOptimizationProfileAsync failed for profile index 0");
    }
  ...
  RuntimeExecState& getOrCreateRuntimeExecState() {
    RuntimeExecState& runtimeState = runtimeExecState;
    if(runtimeState.exec == nullptr) {
      ...
      initializeExecutionContext(runtimeState.exec.get(), &runtimeState.auxStreams);
      runtimeState.lastInputShapeBatch = 0;
    }
    return runtimeState;
  }

  void setInputShapeChecked(IExecutionContext* targetExec, const char* tensorName, Dims dims) {
    bool ok = targetExec->setInputShape(tensorName, dims);
    if(!ok) {
      throw StringError("TensorRT backend: setInputShape failed for tensor " + string(tensorName));
    }
  }

  void validateOutputShapesAfterInputShape(IExecutionContext* targetExec) {
    for(int i = 0; i < engine->getNbIOTensors(); i++) {
      const char* name = engine->getIOTensorName(i);
      if(engine->getTensorIOMode(name) != TensorIOMode::kOUTPUT) {
        continue;
      }
      Dims outDims = targetExec->getTensorShape(name);
      if(outDims.nbDims == -1) {
        throw StringError("TensorRT backend: unresolved output shape after setInputShape for tensor " + string(name));
      }
    }
  }
  ...
  void setInputShapesForExec(IExecutionContext* targetExec, int batchSize) {
    if(ctx->isOnnx) {
      setInputShapeChecked(targetExec, "input_spatial", getBufferDynamicShape("input_spatial", batchSize));
      setInputShapeChecked(targetExec, "input_global", getBufferDynamicShape("input_global", batchSize));
    }
    else {
      setInputShapeChecked(targetExec, "InputMask", getBufferDynamicShape("InputMask", batchSize));
      setInputShapeChecked(targetExec, "InputSpatial", getBufferDynamicShape("InputSpatial", batchSize));
      setInputShapeChecked(targetExec, "InputGlobal", getBufferDynamicShape("InputGlobal", batchSize));
      if(hasInputMetaTensor) {
        setInputShapeChecked(targetExec, "InputMeta", getBufferDynamicShape("InputMeta", batchSize));
      }
    }
    validateOutputShapesAfterInputShape(targetExec);
  }

  BatchGraphState& getOrCreateBatchGraphState(int batchSize) {
    auto search = batchGraphStates.find(batchSize);
    if(search != batchGraphStates.end()) {
      return search->second;
    }

    BatchGraphState state;
    auto inserted = batchGraphStates.emplace(batchSize, std::move(state));
    return inserted.first->second;
  }

  [[noreturn]] void failCudaGraph(const string& message) {
    destroyAllBatchGraphs();
    if(logger != nullptr) {
      logger->write("TensorRT backend: cudaGraph fatal error. " + message);
    }
    throw StringError("TensorRT backend: cudaGraph failure: " + message);
  }

  void enqueueInputCopies(InputBuffers* inputBuffers, int batchSize);
  void enqueueOutputCopies(InputBuffers* inputBuffers, int batchSize);
  void captureCudaGraph(BatchGraphState& state, IExecutionContext* targetExec, InputBuffers* inputBuffers, int batchSize);
  void preCaptureAllCudaGraphs(InputBuffers* inputBuffers);
  void ensureCudaGraphsPreCaptured(InputBuffers* inputBuffers);

  IExecutionContext* getExecutionContextForBatch(int batchSize) {
    validateBatchSize(batchSize);
    RuntimeExecState& runtimeState = getOrCreateRuntimeExecState();
    return runtimeState.exec.get();
  }

  bool shouldSetInputShapeForBatch(int batchSize) {
    validateBatchSize(batchSize);
    RuntimeExecState& runtimeState = getOrCreateRuntimeExecState();
    if(!useCudaGraph) {
      return runtimeState.lastInputShapeBatch != batchSize;
    }
  ...
  void markInputShapeInitializedForBatch(int batchSize) {
    if(useCudaGraph) {
      return;
    }
    validateBatchSize(batchSize);
    RuntimeExecState& runtimeState = getOrCreateRuntimeExecState();
    runtimeState.lastInputShapeBatch = batchSize;
  }

  void enqueueWithOptionalCudaGraph(int batchSize) {
    if(!useCudaGraph) {
      IExecutionContext* targetExec = getExecutionContextForBatch(batchSize);
      targetExec->enqueueV3(cudaStreamPerThread);
      return;
    }
...
ComputeHandle* NeuralNet::createComputeHandle(
  ...
  auto handle = new ComputeHandle(logger, &prop, context, loadedModel, maxBatchSize, requireExactNNLen, serverThreadIdx);
  ...
  if(logger != NULL) {
    logger->write(
      ...
      " useCudaGraph = " + Global::boolToString(handle->ctx->useCudaGraph));
    logger->write(
      "TensorRT backend thread " + Global::intToString(serverThreadIdx) +
...
struct InputBuffers {
  ...
  PinnedHostFloatArray maskInputs;           // Host pointer
  PinnedHostFloatArray spatialInputs;        // Host pointer
  PinnedHostFloatArray globalInputs;  // Host pointer
  PinnedHostFloatArray metaInputs;  // Host pointer
  PinnedHostFloatArray policyPassResults;    // Host pointer
  PinnedHostFloatArray policyResults;        // Host pointer
  PinnedHostFloatArray valueResults;         // Host pointer
  PinnedHostFloatArray scoreValueResults;    // Host pointer
  PinnedHostFloatArray ownershipResults;     // Host pointer

  PinnedHostFloatArray out_policyResults;
  PinnedHostFloatArray out_valueResults;
  PinnedHostFloatArray out_miscvalueResults;
  PinnedHostFloatArray out_moremiscvalueResults;
  PinnedHostFloatArray out_ownershipResults;
  ...
  InputBuffers(const LoadedModel* loadedModel, int maxBatchSz, int nnXLen, int nnYLen) {
    ...
    if (isOnnx) {
        ...
        out_policyResults = makePinnedHostFloatArray(maxBatchSize * singleout_policyElts);
        out_valueResults = makePinnedHostFloatArray(maxBatchSize * singleout_valueElts);
        out_miscvalueResults = makePinnedHostFloatArray(maxBatchSize * singleout_miscvalueElts);
        out_moremiscvalueResults = makePinnedHostFloatArray(maxBatchSize * singleout_moremiscvalueElts);
        out_ownershipResults = makePinnedHostFloatArray(maxBatchSize * singleout_ownershipElts);
    }
    ...
    maskInputs = makePinnedHostFloatArray(maxBatchSize * singleMaskElts);
    spatialInputs = makePinnedHostFloatArray(maxBatchSize * singleInputElts);
    globalInputs = makePinnedHostFloatArray(maxBatchSize * singleInputGlobalElts);
    metaInputs = makePinnedHostFloatArray(maxBatchSize * singleInputMetaElts);
    policyPassResults = makePinnedHostFloatArray(maxBatchSize * singlePolicyPassResultElts);
    policyResults = makePinnedHostFloatArray(maxBatchSize * singlePolicyResultElts);
    valueResults = makePinnedHostFloatArray(maxBatchSize * singleValueResultElts);
    scoreValueResults = makePinnedHostFloatArray(maxBatchSize * singleScoreValueResultElts);
    ownershipResults = makePinnedHostFloatArray(maxBatchSize * singleOwnershipResultElts);
  }
...
void ComputeHandle::enqueueInputCopies(InputBuffers* inputBuffers, int batchSize) {
  if(ctx->isOnnx) {
    CUDA_ERR(
      "enqueueInputCopies",
      cudaMemcpyAsync(
        getBuffer("input_spatial"),
    ...
    if(inputBuffers->singleInputMetaBytes > 0 && hasInputMetaTensor) {
      CUDA_ERR(
        "enqueueInputCopies",
        cudaMemcpyAsync(
          getBuffer("InputMeta"),
...
void ComputeHandle::enqueueOutputCopies(InputBuffers* inputBuffers, int batchSize) {
  if(ctx->isOnnx) {
    CUDA_ERR(
      "enqueueOutputCopies",
      cudaMemcpyAsync(
        inputBuffers->out_policyResults.get(),
        getBuffer("out_policy"),
...
void ComputeHandle::captureCudaGraph(
  BatchGraphState& state,
  IExecutionContext* targetExec,
  InputBuffers* inputBuffers,
  int batchSize
) {
  auto abortCaptureAndDestroyGraph = [&]() {
  ...
  cudaError_t status = cudaStreamBeginCapture(cudaStreamPerThread, cudaStreamCaptureModeThreadLocal);
  if(status != cudaSuccess) {
    failCudaGraph(string("cudaStreamBeginCapture failed: ") + cudaGetErrorString(status));
  }

  try {
    enqueueInputCopies(inputBuffers, batchSize);

    bool enqueueOk = targetExec->enqueueV3(cudaStreamPerThread);
    if(!enqueueOk) {
      throw StringError("enqueueV3 failed while capturing");
    }

    enqueueOutputCopies(inputBuffers, batchSize);
  }
  catch(const StringError& e) {
    abortCaptureAndDestroyGraph();
    failCudaGraph(
      "capture failed for batch size " + Global::intToString(batchSize) + ": " + string(e.what())
    );
  }

  cudaGraph_t capturedGraph = nullptr;
  status = cudaStreamEndCapture(cudaStreamPerThread, &capturedGraph);
  if(status != cudaSuccess || capturedGraph == nullptr) {
    if(capturedGraph != nullptr) {
      (void)cudaGraphDestroy(capturedGraph);
    }
    failCudaGraph(string("cudaStreamEndCapture failed: ") + cudaGetErrorString(status));
  }

  cudaGraphExec_t capturedGraphExec = nullptr;
  status = cudaGraphInstantiate(&capturedGraphExec, capturedGraph, 0);
  if(status != cudaSuccess || capturedGraphExec == nullptr) {
    (void)cudaGraphDestroy(capturedGraph);
    failCudaGraph(string("cudaGraphInstantiate failed: ") + cudaGetErrorString(status));
  }

  destroyBatchGraph(state);
  state.graph = capturedGraph;
  state.graphExec = capturedGraphExec;
}

void ComputeHandle::preCaptureAllCudaGraphs(InputBuffers* inputBuffers) {
  if(!useCudaGraph) {
    return;
  }
  ...
  for(int batchSize = 1; batchSize <= maxBatchSize; batchSize++) {
    ...
    RuntimeExecState& runtimeState = getOrCreateRuntimeExecState();
    setInputShapesForExec(runtimeState.exec.get(), batchSize);
    runtimeState.lastInputShapeBatch = batchSize;

    try {
      enqueueInputCopies(inputBuffers, batchSize);
      bool enqueueOk = runtimeState.exec->enqueueV3(cudaStreamPerThread);
      if(!enqueueOk) {
        throw StringError(
          "enqueueV3 warmup failed while pre-capturing batch size " + Global::intToString(batchSize)
        );
      }
      enqueueOutputCopies(inputBuffers, batchSize);
    }
      ...
      failCudaGraph(
        "warmup failed while pre-capturing batch size " + Global::intToString(batchSize) + ": " + string(e.what())
      );
    }

    CUDA_ERR("preCaptureAllCudaGraphs", cudaStreamSynchronize(cudaStreamPerThread));
    captureCudaGraph(state, runtimeState.exec.get(), inputBuffers, batchSize);
  }

  if(logger != nullptr) {
    logger->write("TensorRT backend: pre-capture completed");
  }
}

void ComputeHandle::ensureCudaGraphsPreCaptured(InputBuffers* inputBuffers) {
  if(!useCudaGraph) {
    return;
  }

  if(cudaGraphsPreCaptured) {
    if(cudaGraphInputBuffers != inputBuffers) {
      failCudaGraph("cudaGraph was captured with a different InputBuffers instance");
    }
    return;
  }

  preCaptureAllCudaGraphs(inputBuffers);
  cudaGraphInputBuffers = inputBuffers;
  cudaGraphsPreCaptured = true;
}

InputBuffers* NeuralNet::createInputBuffers(const LoadedModel* loadedModel, int maxBatchSize, int nnXLen, int nnYLen) {
  InputBuffers* inputBuffers = new InputBuffers(loadedModel, maxBatchSize, nnXLen, nnYLen);
  // Remember this thread's stable InputBuffers pointer for createComputeHandle pre-capture.
  threadLocalInputBuffersForCudaGraph = inputBuffers;
  return inputBuffers;
}

void NeuralNet::freeInputBuffers(InputBuffers* inputBuffers) {
  // Clearing this is defensive for thread reuse after NNServerBuf teardown.
  if(threadLocalInputBuffersForCudaGraph == inputBuffers) {
    threadLocalInputBuffersForCudaGraph = nullptr;
  }
  delete inputBuffers;
}
...
void NeuralNet::getOutput(
  ...
  IExecutionContext* batchExec = gpuHandle->getExecutionContextForBatch(batchSize);
  const bool shouldSetInputShapes = gpuHandle->shouldSetInputShapeForBatch(batchSize);
  ...
  if (isOnnx) {
      ...
      if(shouldSetInputShapes) {
        ...
        batchExec->setInputShape("input_spatial", spatialInputDims);
        batchExec->setInputShape("input_global", globalInputDims);
      }
  } else {
      ...
      if(shouldSetInputShapes) {
        ...
        batchExec->setInputShape("InputMask", maskInputDims);
        batchExec->setInputShape("InputSpatial", spatialInputDims);
        batchExec->setInputShape("InputGlobal", globalInputDims);

        if(numMetaFeatures > 0) {
          auto metaInputDims = gpuHandle->getBufferDynamicShape("InputMeta", batchSize);
          batchExec->setInputShape("InputMeta", metaInputDims);
        }
      }
  }
